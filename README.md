...
Binary distributions of the release 0.4.0 of Clafer Tools for Windows, Mac, and Linux, can be downloaded from [Clafer Tools - Binary Distributions](http://http://gsd.uwaterloo.ca/clafer-tools-binary-distributions).
...
Optional
--------
* [Clafer compiler](https://github.com/gsdlab/clafer) - This backend provides an API for solving Clafer models. The Clafer compiler can compile a Clafer model down to the proper API calls. Can also be done manually by hand quite easily with a bit of typing (examples down below). v0.4.0.
...
```
The CLI is installed to target/chocosolver-0.4.0-jar-with-dependencies.jar. Start the CLI using the command "java -jar chocosolver-0.4.0-jar-with-dependencies.jar mymodel.js".
