scope({c0_AnalogWire:5, c0_AnalysisFunction:4, c0_CAN:2, c0_Command:6, c0_DeviceNode:6, c0_DeviceWire:4, c0_Feature:3, c0_FlexRay:2, c0_FunctionalAnalysisComponent:8, c0_FunctionalDevice:4, c0_LIN:2, c0_Latency:12, c0_LatencyRequirement:17, c0_LogicalDataConnector:2, c0_PhysicalBus:2, c0_Wire:7, c0_WireConfigSegment:10, c0_bus:2, c0_cost:6, c0_deployedFrom:12, c0_deployedTo:8, c0_discreteWire:2, c0_electronic:6, c0_gateway:6, c0_gatewayLatency:6, c0_latency:8, c0_latencyReq:3, c0_length:7, c0_mass:6, c0_minLatency:4, c0_power:6, c0_realizedBy:2, c0_realizes:2, c0_receiver:6, c0_reliability:6, c0_replaceCost:6, c0_sender:6, c0_size:6, c0_smart:6, c0_speedFactor:6, c0_type:6, c0_warrantyCost:6, c1_deployedFrom:12, c1_deployedTo:6, c1_latency:2, c1_latencyReq:8, c1_length:10, c1_mass:7, c1_type:2, c2_latency:2, c2_latencyReq:6, c2_length:2, c2_mass:2, c2_type:2});
defaultScope(1);
intRange(-8, 1500);
stringLength(16);
...
Constraint(implies(some(global(c0_totalCarMass)), equal(joinRef(global(c0_totalCarMass)), add(add(joinRef(join(global(c0_DriverWinSys), c0_totalMass)), joinRef(join(join(global(c0_Car), c1_EC), c0_mass))), joinRef(join(join(global(c0_Car), c1_BCM), c0_mass))))));
Constraint(implies(some(global(c0_totalCarCost)), equal(joinRef(global(c0_totalCarCost)), add(add(joinRef(join(global(c0_DriverWinSys), c0_totalCost)), joinRef(join(join(global(c0_Car), c1_BCM), c0_cost))), joinRef(join(join(global(c0_Car), c1_EC), c0_cost))))));
Constraint(implies(some(global(c0_totalCarWarrantyCost)), equal(joinRef(global(c0_totalCarWarrantyCost)), add(add(joinRef(join(global(c0_DriverWinSys), c0_totalWarrantyCost)), joinRef(join(join(global(c0_Car), c1_BCM), c0_warrantyCost))), joinRef(join(join(global(c0_Car), c1_EC), c0_warrantyCost))))));
