...
install:
	...
	cp -f target/chocosolver-0.4.3-jar-with-dependencies.jar $(to)/chocosolver.jar

snapshotInstall:
	mkdir -p $(to)
	cp -f README.md $(to)/ClaferChocoIG-README.md
	cp -f target/chocosolver-0.4.3-SNAPSHOT-jar-with-dependencies.jar $(to)/chocosolver.jar
